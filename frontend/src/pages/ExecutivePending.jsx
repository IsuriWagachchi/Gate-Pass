...
const ViewExecutivePending = () => {
  ...
  const handleUpdateStatus = async (newStatus) => {
    try {
      await axios.put(`http://localhost:5000/api/executive/${id}/status`, { status: newStatus });
      setStatus(newStatus);
    } catch (error) {
    ...
    <div className="container mx-auto p-6 font-sans flex justify-center">
      <div className="bg-white border-2 border-blue-500 p-6 rounded-lg shadow-lg w-full max-w-3xl mt-6">
        ...
        <div className="p-3 rounded-lg shadow-md border border-gray-300">
          ...
          {status === "Pending" && (
            <>
            <label className="block font-bold mb-2 text-blue-700 mt-5">Comment</label>
            <textarea className="w-full border border-gray-300 p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter Comment Here"></textarea>
            <div className="flex justify-end mt-4 space-x-2">
              <button className="bg-green-600 text-white px-6 py-2 rounded-lg shadow-md" onClick={() => handleUpdateStatus("Approved")}>Approve</button>
