        ...
        <table className="w-full min-w-max border-collapse border rounded-lg shadow-lg">
            <thead>
              <tr className="bg-[#2A6BAC] text-white text-lg">
                ...
                {filter === 'Pending' && <th className="py-3 px-4 border text-center">Actions</th>} */}
              </tr>
            </thead>
            <tbody>
              {requests.filter(req => req.status === filter).map((request, index) => (
                <tr key={request._id} className={`${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}`}>
                  ...
                  {/* <td className={`py-2 px-4 border text-left font-bold ${request.status === 'Approved' ? 'text-green-600' : request.status === 'Rejected' ? 'text-red-600' : 'text-yellow-600'}`}> {request.status} </td>
                  {filter === 'Pending' && (
                    <td className="py-2 px-4 border text-center">
                      <button
                        className="bg-green-500 hover:bg-green-700 text-white px-4 py-1 rounded mr-2 transition-all duration-200"
                        onClick={() => handleUpdateStatus(request._id, 'Approved')}
                      >
                        Approve
                      </button>
                      <button
                        className="bg-red-500 hover:bg-red-700 text-white px-4 py-1 rounded transition-all duration-200"
